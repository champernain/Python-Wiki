...
while True:
    try:
        file_input = xlrd.open_workbook('E:/WingIDE/50ETF20150302.xlsx')     # load market data
    except:
        print 'Input file is not in the directory!'
    break
Market_data = file_input.sheet_by_name('Sheet1')        #input excel sheet1, vol data
...
while True:
    try:
        strike_spreads.append(Market_data.cell(1,3+j).value)
        j = j+1
    except:
        break
...
while True:
    try:
        expiries.append((Market_data.cell(2+i,1).value)/360)
        i = i + 1
    except:
        break
...
while True:
    try:
        optmode.append(Market_data.cell(2+i,0).value)
        i = i + 1
    except:
        break


# to create the opt prices
F = []
i=0
while True:
    try:
...
for i in range(len(expiries)):
    exp_dates[i] = str(int(expiries[i]*360))+'d'

ten_dates = len(optmode)*[0]
